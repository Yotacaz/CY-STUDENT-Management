...
#define DEFINE_DYN_TABLE(Type, Name)                                                                                                                         \
    ...
    {                                                                                                                                                        \
        assert(dtab && file);                                                                                                                                \
        verify(fwrite(&(dtab->capacity), sizeof(int), 1, file) == 1, "couldn't save dynamic table capacity (int) while saving to binary");                   \
        verify(fwrite(&(dtab->size), sizeof(int), 1, file) == 1, "couldn't save dynamic table size (int) while saving to binary");                           \
        if (save_elem == NULL)                                                                                                                               \
        {                                                                                                                                                    \
            /*We assume that if save_elem is NULL, the table does not contain any pointer*/                                                                  \
            verify(fwrite(dtab->tab, sizeof(Type), dtab->size, file) == (size_t)dtab->size, "couldn't save dynamic table content while saving to binary");   \
        }                                                                                                                                                    \
        else                                                                                                                                                 \
        {                                                                                                                                                    \
    ...
    {                                                                                                                                                        \
        ...
        assert(dtab);                                                                                                                                        \
        verify(fread(&(dtab->capacity), sizeof(int), 1, file) == 1, "couldn't load dynamic table capacity (int) while loading from binary");                 \
        verify(fread(&(dtab->size), sizeof(int), 1, file) == 1, "couldn't load dynamic table size (int) while loading from binary");                         \
                                                                                                                                                             \
        ...
        {                                                                                                                                                    \
            ...
            verify(fread(dtab->tab, sizeof(Type), dtab->size, file) == (size_t)dtab->size, "couldn't load dynamic table content while loading from binary"); \
        }                                                                                                                                                    \
        else                                                                                                                                                 \
        {                                                                                                                                                    \
    ...
    {                                                                                                                                                        \
        ...
        if (size < 0 || cap < size)                                                                                                                          \
        {                                                                                                                                                    \
            fprintf(stderr, BOLD_RED "WARNING : dtab of type <%s*> has invalid size (%d) and/or capacity(%d)\n" RESET, #Name, size, cap);                    \
            return false;                                                                                                                                    \
        }                                                                                                                                                    \
        if (size > 0)                                                                                                                                        \
        {                                                                                                                                                    \
            if (!tab)                                                                                                                                        \
            {                                                                                                                                                \
                fprintf(stderr, BOLD_RED "WARNING : dtab of type <%s*> has invalid table (of type %s*)\n" RESET, #Name, #Type);                              \
                return false;                                                                                                                                \
            }                                                                                                                                                \
            if (elem_is_valid)                                                                                                                               \
            {                                                                                                                                                \
                ...
                {                                                                                                                                            \
                    ...
                    {                                                                                                                                        \
                        fprintf(stderr, BOLD_RED "WARNING : table entry %d (type %s*) of dtab (type %s*) is invalid\n" RESET, i, #Type, #Name);              \
                        return false;                                                                                                                        \
                    }                                                                                                                                        \
                }                                                                                                                                            \
            }                                                                                                                                                \
        ...
        {                                                                                                                                                    \
            fprintf(stderr, BOLD_RED "WARNING : table (type %s*) of dtab (type %s*) is supposed null if table size is 0\n" RESET, #Type, #Name);             \
            return false;                                                                                                                                    \
        }                                                                                                                                                    \
        return true;                                                                                                                                         \
    }
